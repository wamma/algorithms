...
int main() {
    ...
    while (1) {
        ...
        switch (choice) {
            ...
            case 4: {
                int counterNumber;
                printf("Enter the counter number (1 to %d): ", NUM_TELLERS);
                scanf("%d", &counterNumber);

                if (counterNumber < 1 || counterNumber > NUM_TELLERS) {
                    printf("Invalid counter number. Please try again.\n");
                    break;
                }
            ...
            case 5: {
                ...
                displayQueue(&bank.vipQueue, -1); // VIP queue
                displayQueue(&bank.waitingQueue, -2); // 대기열
                break;
            }
            case 6: {
